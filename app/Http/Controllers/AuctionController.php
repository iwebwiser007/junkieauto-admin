...
class AuctionController extends Controller
{
    ...
    {  
        ...
        if($req != ''){
            ...
            if($req->auc_type == 'Active Auction'){
                ...
                $data['auction'] = Auction::where('status','2')->with(['user','models','makes','bids'])->latest()->get();
            }
            ...
            if($req->auc_type == 'Expired Auction'){
                ...
                $data['auction'] = Auction::where('status','6')->with(['user','models','makes','bids'])->latest()->get();
            }
    ...
    public function request_auction(Request $req)
    {
        $data['used_type'] = Filter_translation::where('filter_id','10')->get();
        if($req != ''){
            $data['headname'] = $req->auc_type;

            if($req->auc_type == 'All' || $req->auc_type == ''){
                
                $data['auction_req'] = Auction::where('status','1')->with(['user','models','makes','uses'])->latest()->get();
            }
            else{
                
                $data['auction_req'] = Auction::where('status','1')->where('used_type',$req->auc_type)->with(['user','models','makes','uses'])->latest()->get();
            }
    ...
    public function auction_detail($id){
        ...
        $data['bids'] = Auction_bids::where('auction_id',$id)->with('user','commission')->latest()->get();
